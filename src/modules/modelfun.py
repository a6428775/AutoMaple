...
model_path = os.path.join(os.path.join(os.path.join(os.path.join(os.path.dirname(os.path.abspath("main.py")), "assets"),"models"),"rune_model_rnn_filtered_cannied"),"saved_model")
...
class modelfun():
    ...
    def sort_by_confidence(self,model, image):
        """
    ...
    def get_boxes(self,model, image):
        """
    ...
    def merge_detection(self,model,image):
        """
        ...
        filtered = self.filter_color(cropped)
        cannied = self.canny(filtered)

        # Isolate the rune box
        height, width, channels = cannied.shape
        boxes = self.get_boxes(model, cannied)
        if len(boxes) == 4:      # Only run further inferences if arrows have been correctly detected
            ...
            lst = self.sort_by_confidence(model, preprocessed)
            ...
            lst = self.sort_by_confidence(model, rotated)
...
# frame = np.array(cv2.imread('./123.jpg'))

# print(time.time() - t)

# arrows = a.merge_detection(model, frame)
